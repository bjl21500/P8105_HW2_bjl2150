...
of sports balls and plastic bottles. 

The number of observations in the dataset trash_wheel is `r dim(trash_wheel)`. The 
key variables in are dumpster, weight tons of trash, and plastic bottles collected.
The number of variables in the precipitation dataset is `r dim(precipitation)`. The 
key variables in this particular data set are the months and years of precipiation 
as well as the toal amount of precipation `r summarize(total_precip, total = sum(total))`.
The median number of spors balls in a dumpster is `r median(sports_2016$sports_balls)`.
...
# created new dataframe in order to calculate median.
 
brfss_spread_2 = filter(brfss_spread, year == 2002) 


median(brfss_spread_2$excellent)
