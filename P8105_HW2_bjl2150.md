...
2.  Every state is represented. There are total variables within the 'locationsabbr' column indicating that all states are represented plus an additional state, which is Washington, D.C.

3.  

brfss\_spread\_2 = filter(brfss\_spread, year == 2002)

median(brfss\_spread\_2$excellent)

str(brfss\_spread)

ggplot(brfss\_spread\_2, aes(x = excellent)) + geom\_histogram()
